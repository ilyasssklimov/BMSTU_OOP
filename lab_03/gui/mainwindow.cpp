...
void MainWindow::on_shift_fig_clicked()
{
    ...
    cout << "SIZE = " << selectedRows.size() << endl;
...
void MainWindow::on_import_fig_clicked()
{
    ...
    try
    {
        ...
        int number = SceneManagerCreator().get_manager()->get_scene()->get_last_id() - 1;
...
void MainWindow::on_set_cam_clicked()
{
    ...
    shared_ptr<BaseCommand> command(new SetCamera(atoi(id.c_str())));
    _facade->execute(command);
