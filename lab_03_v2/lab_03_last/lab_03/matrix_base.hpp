...
public:
    ...
    template <class U>
    friend inline ostream &operator<<(ostream &, const MatrixBase<U> &);
...
T &MatrixBase<T>::Proxy::at(size_t index)
{
    ...
    {
        string message = "Error in Matrix operator at";
...
T &MatrixBase<T>::Proxy::operator[](size_t index)
{
    ...
    {
        string message = "Error in Matrix operator []";
        throw range_error(message);
    }
}
    ...
    : row_count(row_count), col_count(col_count), memory_dump(row_count * col_count)
{
    ...
    {
        string message = "Error in Matrix construct";
        throw range_error(message);
    }
}
...
template <class T>
MatrixBase<T>::MatrixBase(size_t row_count, size_t col_count, initializer_list<T> lst)
    : MatrixBase(row_count, col_count)
{
    size_t size = this->memory_dump <= lst.size() ? this->memory_dump : lst.size();
    typename initializer_list<T>::iterator iter = lst.begin();
...
{
    copy(other._data, other._data + other.memory_dump, this->_data);
}
...
template <class T>
MatrixBase<T> &MatrixBase<T>::operator=(initializer_list<T> lst)
{
    size_t size = this->memory_dump <= lst.size() ? this->memory_dump : lst.size();
    typename initializer_list<T>::const_iterator iter = lst.begin();
...
{
    swap(this->row_count, other.row_count);
    swap(this->col_count, other.col_count);
    swap(this->memory_dump, other.memory_dump);
...
template <class T>
ostream &operator<<(ostream &stream, const MatrixBase<T> &mtx)
{
