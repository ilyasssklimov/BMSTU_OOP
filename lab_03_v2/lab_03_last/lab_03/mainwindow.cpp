...
void MainWindow::on_shift_fig_clicked()
{
    ...
    Point scale(1, 1, 1);
    Point rotate(0, 0, 0);
    shared_ptr<base_command> command(new TransformModel(index, shift, scale, rotate));
...
{
    shared_ptr<base_command> command(new LoadModel(ui->file_name->text().toStdString()));
    try
    {
        _facade->execute_command(command);
...
{
    shared_ptr<base_command> command(new LoadCamera(ui->file_name_camera->text().toStdString()));
    _facade->execute_command(command);
...
void MainWindow::on_shift_cam_clicked()
{
    ...
    Point rotate(0, 0, 0);
    shared_ptr<base_command> command(new TransformCamera(index, shift, rotate));
    _facade->execute_command(command);
...
*/
    ...
    try
    {
        shared_ptr<AbstractFactory> factory = drawer_solution().get_factory();
...
{
    shared_ptr<base_command> command(new DrawScene(drawer));
    _facade->execute_command(command);
}
...
{
    long index = ui->model_select->currentIndex();
    shared_ptr<base_command> command(new DeleteModel(index));
    _facade->execute_command(command);
...
{
    long index = ui->camera_select->currentIndex();
    shared_ptr<base_command> command(new DeleteCamera(index));
    try
    {
        _facade->execute_command(command);
...
{
    shared_ptr<base_command> command(new set_camera(index));
    _facade->execute_command(command);
